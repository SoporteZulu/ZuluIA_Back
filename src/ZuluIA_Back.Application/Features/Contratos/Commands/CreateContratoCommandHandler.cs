...
public class CreateContratoCommandHandler(ContratosService service, IUnitOfWork uow) : IRequestHandler<CreateContratoCommand, Result<long>>
{
    public async Task<Result<long>> Handle(CreateContratoCommand request, CancellationToken ct)
    {
        try
        {
            var contrato = await service.CrearAsync(request, ct);
