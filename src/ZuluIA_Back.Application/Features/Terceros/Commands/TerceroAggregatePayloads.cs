...
public record TerceroPerfilComercialPayload(
...
public class TerceroPerfilComercialPayloadValidator : AbstractValidator<TerceroPerfilComercialPayload>
{
    private static readonly string[] RiesgosValidos = ["NORMAL", "ALERTA", "BLOQUEADO"];

    public TerceroPerfilComercialPayloadValidator()
    {
        RuleFor(x => x.Rubro)
            .MaximumLength(100)
        ...
        RuleFor(x => x.RiesgoCrediticio)
            .Must(x => string.IsNullOrWhiteSpace(x) || RiesgosValidos.Contains(x.Trim().ToUpperInvariant()))
