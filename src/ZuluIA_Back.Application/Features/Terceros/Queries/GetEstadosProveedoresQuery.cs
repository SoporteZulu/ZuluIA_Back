...
/// <summary>
/// Devuelve el catálogo de estados de proveedores, incluyendo su marca de bloqueo operativo.
/// </summary>
public record GetEstadosProveedoresQuery(bool SoloActivos = false)
    : IRequest<IReadOnlyList<EstadoTerceroCatalogoDto>>;

public class GetEstadosProveedoresQueryHandler(
    IRepository<EstadoProveedor> repo)
    : IRequestHandler<GetEstadosProveedoresQuery, IReadOnlyList<EstadoTerceroCatalogoDto>>
{
    public async Task<IReadOnlyList<EstadoTerceroCatalogoDto>> Handle(
        GetEstadosProveedoresQuery request,
