...
/// <summary>
/// Catálogo de estados de proveedores (legacy VB6: ESTADOSPROVEEDORES).
/// </summary>
public class EstadosProveedoresController(IMediator mediator, IApplicationDbContext db)
    : BaseController(mediator)
{
    ...
    {
        var query = db.EstadosProveedores.AsNoTracking();
        if (activo.HasValue)
    ...
    {
        var item = await db.EstadosProveedores.AsNoTracking()
            .Where(x => x.Id == id)
            .Select(x => new { x.Id, x.Codigo, x.Descripcion, x.Bloquea, x.Activo })
    ...
    public async Task<IActionResult> Create([FromBody] EstadoProveedorRequest req, CancellationToken ct)
    {
        var result = await Mediator.Send(new CreateEstadoProveedorCommand(req.Codigo, req.Descripcion, req.Bloquea), ct);
        if (result.IsFailure)
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status409Conflict)]
    public async Task<IActionResult> Update(long id, [FromBody] EstadoProveedorRequest req, CancellationToken ct)
    {
        var result = await Mediator.Send(new UpdateEstadoProveedorCommand(id, req.Codigo, req.Descripcion, req.Bloquea), ct);
        if (result.IsFailure)
        {
...
public record EstadoProveedorRequest(string Codigo, string Descripcion, bool Bloquea);
