...
/// <summary>
/// Catálogo de categorías de clientes (legacy VB6: CATEGORIACLIENTES).
/// </summary>
public class CategoriasClientesController(IMediator mediator, IApplicationDbContext db)
    : BaseController(mediator)
{
    ...
    {
        var query = db.CategoriasClientes.AsNoTracking();
        if (activa.HasValue)
    ...
    {
        var item = await db.CategoriasClientes.AsNoTracking()
            .Where(x => x.Id == id)
            .Select(x => new { x.Id, x.Codigo, x.Descripcion, x.Activa })
    ...
    public async Task<IActionResult> Create([FromBody] CategoriaClienteRequest req, CancellationToken ct)
    {
        var result = await Mediator.Send(new CreateCategoriaClienteCommand(req.Codigo, req.Descripcion), ct);
        if (result.IsFailure)
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status409Conflict)]
    public async Task<IActionResult> Update(long id, [FromBody] CategoriaClienteRequest req, CancellationToken ct)
    {
        var result = await Mediator.Send(new UpdateCategoriaClienteCommand(id, req.Codigo, req.Descripcion), ct);
        if (result.IsFailure)
        {
...
public record CategoriaClienteRequest(string Codigo, string Descripcion);
