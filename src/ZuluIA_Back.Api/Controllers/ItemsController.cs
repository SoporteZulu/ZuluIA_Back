...
public class ItemsController(IMediator mediator) : BaseController(mediator)
{
    [HttpGet]
        ...
        CancellationToken ct = default)
    {
        var result = await Mediator.Send(
            new GetItemsPagedQuery(page, pageSize, search, soloProductos, soloServicios), ct);
        return Ok(result);
    }
        ...
        CancellationToken ct)
    {
        ...
        return CreatedFromResult(result, "GetItemById", new { id = result.IsSuccess ? result.Value : 0 });
    }
