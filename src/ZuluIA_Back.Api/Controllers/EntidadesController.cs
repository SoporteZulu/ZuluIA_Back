...
/// <summary>
/// Módulo de entidades externas/integraciones.
/// Implementado sobre el catálogo de Integradoras.
/// </summary>
[Route("api/entidades")]
public class EntidadesController(IMediator mediator, IApplicationDbContext db) : BaseController(mediator)
{
    [HttpGet]
    ...
    public async Task<IActionResult> Create([FromBody] EntidadCreateRequest req, CancellationToken ct)
    {
        ...
        if (result.IsFailure)
            return result.Error?.Contains("Ya existe", StringComparison.OrdinalIgnoreCase) == true
                ? Conflict(new { error = "Ya existe una entidad con ese codigo." })
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(long id, [FromBody] EntidadUpdateRequest req, CancellationToken ct)
    {
        var result = await Mediator.Send(
        ...
        if (result.IsFailure)
            return result.Error?.Contains("no encontrada", StringComparison.OrdinalIgnoreCase) == true
                ? NotFound(new { error = $"Entidad {id} no encontrada." })
                : BadRequest(new { error = result.Error });
...
public record EntidadCreateRequest(
    string Codigo,
    string Nombre,
    string TipoSistema,
    string? UrlEndpoint,
...
public record EntidadUpdateRequest(
    string Nombre,
    string TipoSistema,
    string? UrlEndpoint,
    string? Configuracion);
