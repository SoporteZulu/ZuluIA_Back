...
public class EstadosClientesControllerTests
{
    [Fact]
    public async Task GetAll_CuandoHayDatos_DevuelveOkConItemsOrdenados()
    {
        ...
        db.EstadosClientes.Returns(estados);
    ...
    public async Task GetById_CuandoNoExiste_DevuelveNotFound()
    {
        var mediator = Substitute.For<IMediator>();
        var db = Substitute.For<IApplicationDbContext>();
        var estados = MockDbSetHelper.CreateMockDbSet<EstadoCliente>();
        db.EstadosClientes.Returns(estados);
        var controller = CreateController(mediator, db);
    ...
    public async Task Create_CuandoFallaPorDuplicado_DevuelveConflict()
    {
        ...
        mediator.Send(Arg.Any<CreateEstadoClienteCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result.Failure<long>("Ya existe un estado con ese codigo."));

        var result = await controller.Create(new EstadoClienteRequest("ACT", "Activo", false), CancellationToken.None);
    ...
    public async Task Create_CuandoFallaPorValidacion_DevuelveBadRequest()
    {
        ...
        var result = await controller.Create(new EstadoClienteRequest("", "Activo", false), CancellationToken.None);
    ...
    public async Task Create_CuandoTieneExito_DevuelveCreatedAtAction()
    {
        ...
        result.Should().BeOfType<CreatedAtActionResult>()
            .Which.ActionName.Should().Be(nameof(EstadosClientesController.GetById));
    }
    ...
    public async Task Update_CuandoNoExiste_DevuelveNotFound()
    {
        var mediator = Substitute.For<IMediator>();
        var controller = CreateController(mediator);
        mediator.Send(Arg.Any<UpdateEstadoClienteCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result.Failure("Estado no encontrado."));

        var result = await controller.Update(8, new EstadoClienteRequest("ACT", "Activo", false), CancellationToken.None);
    ...
    public async Task Update_CuandoFallaPorDuplicado_DevuelveConflict()
    {
        ...
        var controller = CreateController(mediator);
        mediator.Send(Arg.Any<UpdateEstadoClienteCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result.Failure("Ya existe un estado con ese codigo."));
    ...
    public async Task Desactivar_CuandoNoExiste_DevuelveNotFound()
    {
        var mediator = Substitute.For<IMediator>();
        var controller = CreateController(mediator);
        mediator.Send(Arg.Any<DeactivateEstadoClienteCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result.Failure("Estado 8 no encontrado."));
    ...
    public async Task Desactivar_CuandoTieneExito_DevuelveOkYUsaCommand()
    {
        ...
        await mediator.Received(1).Send(
            Arg.Is<DeactivateEstadoClienteCommand>(command => command.Id == 8),
    ...
    public async Task Activar_CuandoNoExiste_DevuelveNotFound()
    {
        var mediator = Substitute.For<IMediator>();
        var controller = CreateController(mediator);
        mediator.Send(Arg.Any<ActivateEstadoClienteCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result.Failure("Estado 8 no encontrado."));
    ...
    public async Task Activar_CuandoTieneExito_DevuelveOkYUsaCommand()
    {
        ...
        result.Should().BeOfType<OkResult>();
        await mediator.Received(1).Send(
            Arg.Is<ActivateEstadoClienteCommand>(command => command.Id == 8),
            Arg.Any<CancellationToken>());
    }

    private static EstadosClientesController CreateController(IMediator mediator, IApplicationDbContext? db = null)
    {
        var controller = new EstadosClientesController(mediator, db ?? Substitute.For<IApplicationDbContext>())
        {
    ...
    private static EstadoCliente BuildEstado(long id, string codigo, string descripcion, bool bloquea, bool activo)
    {
        var entity = EstadoCliente.Crear(codigo, descripcion, bloquea, userId: null);
        typeof(EstadoCliente).BaseType!
            .GetProperty("Id")!
            .SetValue(entity, id);
