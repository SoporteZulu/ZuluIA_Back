...
public class EmpleadosLiquidacionesControllerTests
{
    ...
    [Fact]
    public async Task GetLiquidaciones_CuandoEmpleadoNoExiste_DevuelveNotFoundConError()
    {
        var mediator = Substitute.For<IMediator>();
        ...
        var result = await controller.GetLiquidaciones(7, null, null, CancellationToken.None);
    ...
    public async Task GetLiquidaciones_CuandoHayDatos_DevuelveOkFiltradoYOrdenado()
    {
        ...
        var result = await controller.GetLiquidaciones(7, 2026, null, CancellationToken.None);
    ...
    [Fact]
    public async Task MarcarLiquidacionPagada_CuandoNoExiste_DevuelveNotFoundConMensajeNormalizado()
    {
        var mediator = Substitute.For<IMediator>();
        mediator.Send(Arg.Any<MarcarLiquidacionPagadaCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result.Failure("No se encontro la liquidacion 31 para el empleado 7."));
        var controller = CreateController(mediator);

        var result = await controller.MarcarLiquidacionPagada(7, 31, CancellationToken.None);

        result.Should().BeOfType<NotFoundObjectResult>()
            .Which.Value!.ToString().Should().Contain("No se encontró la liquidación 31 para el empleado 7");
    }

    [Fact]
    public async Task MarcarLiquidacionPagada_CuandoYaEstaPagada_DevuelveBadRequestConMensajeNormalizado()
    {
        var mediator = Substitute.For<IMediator>();
        mediator.Send(Arg.Any<MarcarLiquidacionPagadaCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result.Failure("La liquidacion ya se encuentra marcada como pagada."));
        var controller = CreateController(mediator);

        var result = await controller.MarcarLiquidacionPagada(7, 31, CancellationToken.None);

        result.Should().BeOfType<BadRequestObjectResult>()
            .Which.Value!.ToString().Should().Contain("liquidación ya se encuentra marcada como pagada");
    }

    [Fact]
    public async Task MarcarLiquidacionPagada_CuandoTieneExito_DevuelveOkConMensaje()
    {
        var mediator = Substitute.For<IMediator>();
        mediator.Send(Arg.Any<MarcarLiquidacionPagadaCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result.Success());
        ...
        result.Should().BeOfType<OkObjectResult>()
            .Which.Value!.ToString().Should().Contain("Liquidación marcada como pagada correctamente");
    }
